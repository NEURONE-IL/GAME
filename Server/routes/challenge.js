...
updateChallengeStudySearch = async (study_id) => {
  try {
    ...
    const studySearch = await StudySearch.findOne({ study: study_id });
    //Encontrar los challenges asociados
    const challenges = await Challenge.find({ study: study_id });
...
router.get("/answer/all", [verifyToken], async (req, res) => {
  UserChallenge.find({}, (err, userChallenges) => {
    ...
    .populate({ path: "user", model: User }, { password: 0 })
...
router.post(
  "",
  [verifyToken, authMiddleware.isAdmin, challengeMiddleware.verifyBody],
  async (req, res) => {
    const challenge = new Challenge(req.body);
    challenge.save(async (err, challenge) => {
      if (err) {
        return res.status(404).json({
          err,
        });
      }
      await updateChallengeStudySearch(challenge.study);
      res.status(200).json({
        challenge,
      });
    });
...
router.post(
  "/answer",
  [verifyToken, challengeMiddleware.verifyAnswerBody],
  async (req, res) => {
    ...
    let distance = await normalizeAndDistance(
      challenge.answer.toLowerCase(),
      req.body.answers[0].answer.toLowerCase()
    );
    ...
    userChallenge.save((err, userChallenge) => {
      ...
      if (user && user.gm_code && pointElement) {
        let post = {
          ...
          date: userChallenge.createdAt,
          amount: pointsObtained,
        };
      ...
      if (user && user.gm_code && answerAction) {
        let post = {
          action_code: answerAction.gm_code,
          date: userChallenge.createdAt,
        };
      ...
      if (user && user.gm_code && noHintsAction) {
        let post = {
          action_code: noHintsAction.gm_code,
          date: userChallenge.createdAt,
        };
        actionService.postPlayerAction(post, user.gm_code, (err, data) => {
          if (err) {
...
router.get("/editStatus/:challenge_id/:user_id", async (req, res) => {
  ...
  var id = setInterval(async function () {
    ...
    if (challenge.edit[0] == _user_challenge_id_event) {
      if (challenge.edit.length == 1) Stream.removeAllListeners();
      clearInterval(id);
    }
  }, 10000);
});
...
router.put(
  ...
  async (req, res) => {
    ...
    lock.acquire(
      _challengeRequestEdit,
      async function (done) {
        console.log(_userRequestEditChall + " acquire");

        const challenge = await Challenge.findOne(
          { _id: _challengeRequestEdit },
          (err) => {
            if (err) {
              return res.status(404).json({
                err,
...
router.put(
  "/:challenge_id",
  [verifyToken, authMiddleware.isAdmin, challengeMiddleware.verifyEditBody],
  async (req, res) => {
    const _id = req.params.challenge_id;
    ...
    const challenge = await Challenge.findOne(
      ...
      (err, challenge) => {
        ...
        if (req.body.messages) {
          if (req.body.messages === "-") challenge.messages = "";
          else challenge.messages = req.body.messages;
        }
