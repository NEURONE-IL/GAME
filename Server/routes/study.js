...
router.get("/:study_id", async (req, res) => {
  ...
  Study.findOne({ _id: _id }, (err, study) => {
    ...
    .populate({ path: "competences", model: Competence });
});
...
*/
router.get(
  "/byUserbyPrivacy/:user_id/:privacy",
  [verifyToken],
  async (req, res) => {
...
*/
router.get(
  "/copy/:study_id/user/:user_id/",
  [verifyToken],
  async (req, res) => {
    const _id = req.params.study_id;
    const _user_id = req.params.user_id;
    try {
      const study = await Study.findOne({ _id: _id });

      if (!study)
        return res
          .status(404)
          .json({ msg: "No existe el estudio que se desea clonar" });

      const challenges = await Challenge.find({ study: study._id });
      ...
      await axios
        .post(process.env.NEURONE_URL + "v1/document/search", post, {
        ...
        .catch((err) => {
          console.log("Error al obtener los recursos del estudio", err);
          throw err;
        });
      for await (const challenge of challenges) {
        ...
        let _challenge_id = JSON.stringify(challenge._id);
        let challengeResources = filteredResources.filter(
          (resource) => JSON.stringify(resource.task[0]) === _challenge_id
        );
        let promises = [];
        for await (const resource of challengeResources) {
          let urlResource = "";
          let docName = resource.docName.split("(Clonado)")[0];
          ...
          try {
            urlResource =
              "https://" +
              resource.route.split(resource.docName)[1].split("/")[1];
          } catch (err) {
            console.error("Error al obtener url recurso", resource, err);
            urlResource = resource.route;
          }
          let resourceCopy = {
            docName: docName + " (Clonado) " + timeStamp,
            title: resource.title,
            url: urlResource,
            domain: [copy._id],
            locale: resource.locale,
            task: [newChallenge._id],
            maskedUrl: resource.url,
          ...
          else Object.assign(resourceCopy, { type: "document" });

          promises.push(
            axios
              .post(
                process.env.NEURONE_URL + "v1/document/load",
                resourceCopy,
                {
                  headers: { "Content-Type": "text/plain" }
                }
              ...
              .catch((err) => {
                console.log(resource.docName, err);
              })
          );
        }
      ...
      const userClone = await User.findOne({ _id: _user_id }, { password: 0 });
      ...
      const notification = new AdminNotification({
        ...
        description:
          userClone.names +
          " " +
          userClone.last_names +
          " ha clonado su estudio: " +
          study.name,
        seen: false
      });
      let savePromises = [copyHistory.save(), notification.save(), copy.save()];
      await Promise.all(savePromises);

      groupService.postGroup(
        { name: copy.name, sourceId: copy._id },
        async (err, data) => {
          if (err) {
            res.status(200).json({
              copy
      ...
      console.error("Clonar estudio:", error);
      res
        .status(500)
        .json({ msg: "Ocurrió un error al clonar el estudio", error });
    }
  }
);
...
router.post(
  ...
  [
    ...
    imageStorage.upload.single("file"),
    studyMiddleware.verifyBody
  ],
  async (req, res) => {
    ...
    study.save(async (err, study) => {
      ...
      if (study.privacy == false) {
        await study
          ...
          .execPopulate();
        await createStudySearch(study, [], "createStudy").catch((error) => {
          console.error(error);
          res.status(400).json;
        });
      }
      ...
      groupService.postGroup(
        { name: req.body.name, sourceId: study._id },
        (err, data) => {
          if (err) {
            res.status(200).json({
              study
...
router.get("/editStatus/:study_id/:user_id", async (req, res) => {
  ...
  var id = setInterval(async function () {
    ...
    Stream.emit(_user_id_event + "/" + _study_id_event, "message", {
      currentUsers: study.edit
    });

    if (study.edit[0] == _user_id_event) {
      if (study.edit.length == 1) Stream.removeAllListeners();
      clearInterval(id);
    }
  }, 10000);
});
...
*/
router.put(
  "/requestEdit/:study_id" /*, [verifyToken, authMiddleware.isAdmin]*/,
  async (req, res) => {
...
router.put(
  ...
  async (req, res) => {
    ...
    study.save((err) => {
      ...
      res
        .status(200)
        .json({ message: "Edit list successfully updated", study });
    });
    //await delay(5); Para probar
  }
);
...
router.put(
  ...
  [
    ...
    authMiddleware.isAdmin,
    imageStorage.upload.single("file"),
    studyMiddleware.verifyEditBody
  ],
  async (req, res) => {
    const _id = req.params.study_id;
    ...
    const study = await Study.findOne({ _id: _id }, (err, study) => {
      ...
      if (study.privacy == privacy) privacyChange = false;
      else {
      ...
      study.save(async (err, study) => {
        ...
        if (study.privacy == true && privacyChange) deleteStudySeach(study._id);
        else if (study.privacy == false && privacyChange)
          createStudySearch(study, [], "privacyUpdate");
        else if (study.privacy == false && !privacyChange)
          await updateStudySearch(study);
  ...
  async (req, res) => {
    ...
    if (collaborators) {
      study.collaborators.forEach(async (coll) => {
        ...
        let collDelete = collaborators.some(
          (item) => JSON.stringify(item.user._id) === JSON.stringify(coll.user)
        );

        if (!collDelete && coll.invitation === "Pendiente") {
          console.log("borra");
          await Invitation.findOneAndDelete(
            { user: coll.user, status: "Pendiente", study: study._id },
            async (err, inv) => {
              if (err) {
                return res.status(404).json({
                  ok: false,
      ...
      collaborators.forEach((coll, i) => {
        let index = study.collaborators.findIndex(
          (item) => item.user == coll.user._id
        );
        ...
        if (!(index >= 0)) {
          ...
          const notification = new AdminNotification({
            ...
            description:
              "Invitación para colaborar en el estudio: " + study.name,
            seen: false
          });
          notification.save((err) => {
            if (err) {
...
router.delete(
  ...
  async (req, res) => {
    ...
    Study.findOneAndDelete({ _id: _id }, (err, study) => {
      ...
      if (study.privacy === false) deleteStudySeach(_id);
...
router.get(
  ...
  async (req, res) => {
    ...
    await UserStudy.find({ study: study_id }).exec((err, userStudies) => {
      if (err) {
        return res.status(404).json({
          ok: false,
          err
      ...
      userStudies.forEach((element) => {
        //Get username
        let username;
        User.findOne({ _id: element.user }).exec((err, user) => {
          if (err) {
          ...
          if (user) {
            ...
            //Get last login date
            let lastSession;
            SessionLog.findOne({ userId: user._id, state: "login" }, null, {
              sort: { createdAt: -1 }
            }).exec((err, sessionLog) => {
              if (err) {
                res.status(500).json(err);
              ...
              if (sessionLog) {
                ...
                let answers = element.challenges.filter(
                  (challenge) => challenge.finished === true
                ).length;
...
router.put("/:study_id/assistant", async (req, res) => {
  ...
  if (!study) {
    ...
    const studyAssistant = await StudyAssistant.findOne(
      { study: study._id },
      (err) => {
        if (err) {
          return res.status(404).json({
...
async function deleteStudySeach(study_id) {
  ...
  try {
    ...
    //Encontrar el studySearch y eliminarlo
    await StudySearch.deleteOne({ study: _study_id });
  } catch (err) {
    console.log(err);
    throw err;
  }
...
async function updateStudySearch(study) {
  ...
  try {
    ...
    const studySearch = await StudySearch.findOne({ study: _study_id });
    // let competences = [];
...
async function createStudySearch(study, challenges, type) {
  ...
  try {
    ...
    if (type === "privacyUpdate") {
      challenges = await Challenge.find({ study: _study_id });
    }
