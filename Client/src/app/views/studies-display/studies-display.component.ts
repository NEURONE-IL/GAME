...
import { UserCreationFormComponent } from 'src/app/views/user-creation-form/user-creation-form.component';
...
export class StudiesDisplayComponent implements OnInit {
  ...
  getAllStudiesByUser() {
    this.studyService.getStudiesByUser(this.user._id).subscribe(
      (response) => {
        ...
        if (this.studies.length <= 0) this.noStudies = true;
        else this.noStudies = false;
      },
      (err) => {
        this.toastr.error(
  ...
  openPreview() {
    this.matDialog.open(UserCreationFormComponent, {
      id: 'app-user-creation-form',
      width: '80%',
    });
  }
}
