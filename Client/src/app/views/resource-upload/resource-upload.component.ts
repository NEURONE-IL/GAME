...
export class ResourceUploadComponent implements OnInit {
  ...
  uploadResource() {
    ...
    this.endpointsService.loadDocument(resource).subscribe(
      (resource) => {
        this.toastr.success(
          this.translate.instant('UPLOAD.TOAST.SUCCESS_MESSAGE'),
      ...
      (err) => {
        this.loading = false;
        //No se está retornando el mensaje de error
        // if (err.error.msg === 'COULDNT_DOWNLOAD_DOC') {
        //   this.toastr.error(
        //     this.translate.instant('UPLOAD.TOAST.ERROR_' + err.error.msg),
        ...
        // } else {
        this.toastr.error(
          this.translate.instant('UPLOAD.TOAST.ERROR_MESSAGE'),
          this.translate.instant('UPLOAD.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
