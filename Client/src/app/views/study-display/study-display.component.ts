...
import {
  ...
  Study,
  StudyService,
} from '../../services/game/study.service';
import {
  EndpointsService,
  Resource,
} from '../../services/endpoints/endpoints.service';
...
export class StudyDisplayComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.studyService
      .getStudy(this.route.snapshot.paramMap.get('study_id'))
      .subscribe(
        (response) => {
          this.study = response['study'];
          this.emailFormControl = new FormControl('', [
            Validators.email,
            notThisUser(this.user),
            notExistingColl(this.study.collaborators),
          ]);

          let _user_id = this.user._id;

          this.registerLink =
            environment.frontURL + 'welcome/' + this.study._id;
        ...
        (err) => {
          this.toastr.error(
            this.translate.instant('STUDY.TOAST.NOT_LOADED_ERROR'),
    ...
    this.challengeService
      .getChallengesByStudy(this.route.snapshot.paramMap.get('study_id'))
      .subscribe((response) => {
        this.challenges = response['challenges'];
      });

    this.endpointsService
      .getDocuments('*', this.route.snapshot.paramMap.get('study_id'))
    ...
    this.historyService
      .getHistoryByStudyByType(
        this.route.snapshot.paramMap.get('study_id'),
        'clone'
      )
      .subscribe(
        (response) => {
      ...
      (response) => {
        ...
        this.users.forEach((element) => {
          if (element.lastSession) {
            element.lastSession = new Date(element.lastSession).toLocaleString(
              'es-CL'
            );
          }
        });
  ...
  findDummy() {
    this.authService.findDummy(this.study._id).subscribe((response) => {
  ...
  createDummy() {
    this.authService.signupDummy(this.study._id).subscribe(
      (user) => {
        this.toastr.success(
          this.translate.instant('STUDY.TOAST.DUMMY_SUCCESS'),
          this.translate.instant('STUDY.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('STUDY.TOAST.DUMMY_ERROR'),
          this.translate.instant('STUDY.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
  ...
  renewDummy() {
    this.authService.renewDummy(this.study._id).subscribe(
      (user) => {
        this.toastr.success(
          this.translate.instant('STUDY.TOAST.DUMMY_RESET_SUCCESS'),
          this.translate.instant('STUDY.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('STUDY.TOAST.DUMMY_RESET_ERROR'),
          this.translate.instant('STUDY.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  confirmStudyDelete(id: string) {
    confirm(this.translate.instant('ADMIN.STUDIES.DELETE_CONFIRMATION')) &&
      this.deleteStudy(id);
  }
  confirmCollaborationLeft() {
    confirm(
      'Seguro que desea dejar de ser colaborador del estudio: ' +
        this.study.name
    ) && this.collaborationLeft();
  }
  collaborationLeft() {
    let collaborators = this.study.collaborators.slice();
    let index = collaborators.findIndex(
      (coll) => coll.user._id === this.user._id
    );
    collaborators.splice(index, 1);
    console.log(collaborators);
    this.editCollaborator(
      collaborators,
      'Ha dejado de ser colaborador del estudio: ' + this.study.name,
      'No se ha podido realizar la operación, intente más tarde'
    );
  ...
  deleteStudy(id: string) {
    this.studyService.deleteStudy(id).subscribe(
      (study) => {
        this.toastr.success(
          this.translate.instant('STUDY.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('STUDY.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('STUDY.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('STUDY.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  getChallengeResources(challengeId: string) {
    var finalResources = [];
    var filterByChallengeResources = this.filteredResources.filter(
      (resource) => resource.task[0] === challengeId
    );
    filterByChallengeResources.forEach((resource) =>
      finalResources.push(resource)
    );
    return finalResources;
  }
  confirmAddCollaborator() {
    confirm('¿Seguro que desea agregar al colaborador?') &&
      this.verifyCollaborator();
  }
  confirmRemoveCollaborator(collaborator) {
    confirm('¿Seguro que desea eliminar al colaborador?') &&
  ...
  deleteCollaborator(collaborator) {
    ...
    this.editCollaborator(
      newCollaboratorList,
      'Se ha eliminado correctamente el colaborador al estudio',
      'El colaborador no ha podido ser eliminado'
    );
  }
  addCollaborator(user: User) {
    ...
    this.editCollaborator(
      newCollaboratorList,
      'Se ha añadido correctamente el colaborador al estudio',
      'El colaborador no ha podido ser añadido'
    );
  ...
  editCollaborator(collaboratorList, msg1, msg2) {
    this.studyService
      .editCollaboratorStudy(this.study._id, collaboratorList)
      .subscribe(
        (response) => {
          ...
          } else this.collaboratorsExist = false;
        },
        (err) => {
    ...
    this.authService.getUserbyEmail(this.emailFormControl.value).subscribe(
      ...
      (error) => {
        ...
        if (error === 'ROLE_INCORRECT') {
          this.toastr.error(
            'El usuario ingresado no cuenta con permisos de colaborador',
            'Usuario Incorrecto',
            {
              timeOut: 5000,
              positionClass: 'toast-top-center',
        ...
        if (error === 'USER_NOT_CONFIRMED') {
          this.toastr.error(
            'El usuario ingresado no ha terminado su proceso de registro',
  ...
  confirmChallengeDelete(id: string) {
    confirm(this.translate.instant('ADMIN.CHALLENGES.DELETE_CONFIRMATION')) &&
  ...
  deleteChallenge(id: string) {
    this.challengeService.deleteChallenge(id).subscribe(
      (challenge) => {
        ...
        this.toastr.success(
          this.translate.instant('CHALLENGE.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('CHALLENGE.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('CHALLENGE.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('CHALLENGE.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
  ...
  confirmResourceDelete(resource: Resource) {
    confirm(
      this.translate.instant('ADMIN.CHALLENGES.RESOURCE_DELETE_CONFIRMATION')
    ) && this.deleteResource(resource);
  }

  deleteResource(resource: Resource) {
    this.deletingResource = true;
    this.endpointsService.deleteDocument(resource).subscribe(
      (response) => {
        this.endpointsService
          .getDocuments('*', this.route.snapshot.paramMap.get('study_id'))
          .subscribe((response: Resource[]) => {
        ...
        this.toastr.success(
          this.translate.instant('UPLOAD.TOAST.SUCCESS_MESSAGE_DELETE'),
          this.translate.instant('UPLOAD.TOAST.SUCCESS'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('UPLOAD.TOAST.ERROR_MESSAGE_DELETE'),
          this.translate.instant('UPLOAD.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          }
  ...
  updateChallenge(id: string, updatedChallenge: string) {
    this.challengeService.putChallenge(id, updatedChallenge).subscribe(
      (challenge) => {
        this.challengeService
          .getChallengesByStudy(this.route.snapshot.paramMap.get('study_id'))
          .subscribe((response) => (this.challenges = response['challenges']));
        this.toastr.success(
          this.translate.instant('CHALLENGE.TOAST.SUCCESS_MESSAGE_UPDATE') +
      ...
      (err) => {
        this.toastr.error(
          this.translate.instant('CHALLENGE.TOAST.ERROR_MESSAGE_UPDATE'),
          this.translate.instant('CHALLENGE.TOAST.ERROR'),
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
          ...
          dialogRef.afterOpened().subscribe(() => {
            setTimeout(() => {
              if (!(dialogRef.getState() === MatDialogState.CLOSED)) {
                this.toastr.info(
                  'Se ha acabado su tiempo de edición',
                  'Información',
                  {
                    timeOut: 5000,
                    positionClass: 'toast-top-center',
  ...
  requestEdit() {
    this.studyService
      .requestForEdit(this.study._id, { user: this.user._id })
      .subscribe(
        (response) => {
  ...
  cloneStudy() {
    ...
    this.toastr.warning(
      'El proceso puede demorar según la cantidad de recursos que hayan enlazados, por favor no cierre esta ventana',
      'Advertencia',
      {
        timeOut: 15000,
        positionClass: 'toast-top-center',
    ...
    this.studyService.copyStudy(this.study._id, user_id).subscribe(
      (response) => {
        ...
        this.toastr.success(
          'El estudio ha sido clonado exitosamente, los recursos pueden tardar unos minutos en cargarse',
          'Éxito',
          {
            timeOut: 5000,
            positionClass: 'toast-top-center',
      ...
      (err) => {
        this.toastr.error(
          'El estudio seleccionado no ha podido ser clonado',
