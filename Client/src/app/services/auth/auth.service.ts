...
export class AuthService {
  ...
  login(email: string, password: string) {
    this.http
      .post(this.uri + 'login', { email: email, password: password })
      .subscribe(
        (resp: any) => {
        ...
        (error) => {
          ...
          if (error.error == 'EMAIL_NOT_FOUND') {
            error_msg = this.translate.instant(
              'LOGIN.TOAST.ERROR_EMAIL_MESSAGE'
            );
          } else if (error.error == 'INVALID_PASSWORD') {
            error_msg = this.translate.instant(
              'LOGIN.TOAST.ERROR_CREDENTIALS_MESSAGE'
            );
          } else if (error.error == 'USER_NOT_CONFIRMED') {
            error_msg = this.translate.instant(
              'LOGIN.TOAST.ERROR_USER_NOT_CONFIRMED'
            );
          }
          this.toastr.error(
            error_msg,
  ...
  confirmLogout() {
    confirm(this.translate.instant('LOGOUT.LOGOUT_CONFIRMATION')) &&
      this.logout();
  }
  ...
  registerAPIKEY(email, names, study, trainer_id, api_key, url) {
    ...
    return this.http
      .post(
        environment.apiURL + 'site/registeruser',
        {
          email: email,
          names: names,
          last_names: names,
  ...
  loginAPIKEY(study, trainer_id, api_key, url) {
    ...
    return this.http
      .post(
        environment.apiURL + 'site/login',
        { study: study, trainer_id: trainer_id, url: url },
        { headers: headers }
      )
      .subscribe(
        (resp: any) => {
  ...
  getUserByRole(user_id, role_id) {
    return this.http.get(
      this.userUri + 'getUserByRole/' + user_id + '/' + role_id
    );
  }
  ...
  refreshUser() {
    return new Promise((resolve, reject) => {
      this.http.put(this.userUri + this.getUser()._id, {}).subscribe(
        (res: any) => {
  ...
  refreshProgress() {
    return new Promise((resolve, reject) => {
      this.http
        .get(this.userUri + this.getUser()._id + '/progress', {})
        .subscribe(
          (progress: any) => {
            resolve(progress);
          },
  ...
  updateProgress(body) {
    return new Promise((resolve, reject) => {
      this.http
        .put(this.userUri + this.getUser()._id + '/progress', body)
        .subscribe(
          (progress: any) => {
            resolve(progress);
          },
  ...
  canPlay() {
    return new Promise((resolve, reject) => {
      this.http.get(this.userUri + this.getUser()._id + '/can_play').subscribe(
        (canPlay: any) => {
          resolve(canPlay);
        },
        (error) => {
  ...
  postForward(course, lastLink) {
    return this.http.post(environment.apiURL + 'forward', {
      course: course,
      lastLink: lastLink,
    });
  }
